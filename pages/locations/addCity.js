...
import {countryList,stateList,cityList,addCity,editCity,deleteLocation} from '../../actions/locationsActions'
...
const Cities = () => {
    ...
    const { name, error, success,type,country_id,city_id, cities,state_id,header,states,countries,message, removed } = values;
    ...
    const loadCountries = () => {
                        ...
                        cityList(token).then(cityData => {
                            if (data.error) {
                                ...
                                setValues({ ...values, 
                                    ...
                                    name:'',header:"Add City",
                                    type:"add",country_id:'',state_id:'',city_id:''});
      ...
      const handleSubmit = e => {
        ...
        const chapter = { name,country_id,state_id };

        if(type === "add"){
            addCity(chapter,token).then(data => {
                if (data.error) {
            ...
            const cities = { name,country_id ,state_id,city_id};
            editCity(cities,token).then(data => {
                if (data.error) {
                    setValues({ ...values, error: data.error, loading: false });
                } else {
                    loadCountries();
   ...
   function edit(cell, row){
    const handleClick = e => {
        setValues({ ...values, error: false, name : row.name,header:"Edit City",type:"edit",country_id:row.country_id,city_id:row._id,state_id:row.state_id});
    };
    ...
    <div className="row">
        <div className="col-12">
            <div className="page-title-box">
                <h4 className="page-title float-left">Cities</h4>
    ...
    <div className="card-box">
            ...
            <div   style={{
              ...
              {/* <Link to='/add_category' class="btn btn-primary" style={{float:'left',backgroundColor:'#11abff'}}>Add Chapter</Link> */}
                    ...
                    <BootstrapTable data={cities} cellEdit={cellEditProp } pagination search dataSort striped hover>
                    <TableHeaderColumn width='80px' dataField='sno' dataAlign="center" dataSort isKey>S.No</TableHeaderColumn>
                    <TableHeaderColumn width='180px'  dataField='name' dataAlign="center" editable={false} dataSort> City </TableHeaderColumn>
