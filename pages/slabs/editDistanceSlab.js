...
import {editDistance,getDistanceById,cityList} from '../../actions/distanceAction'
...
const EditDistance = () => {
    ...
    const loadCities = () => {
        cityList(token).then(data => {
            if (data.error) {
                ...
                getDistanceById(token,router.query._id).then(distance => {
                    if (distance.error) {
                        console.log(data.error);
                    } else {
                        setValues({ ...values, 
                            ...
                            rate:distance.rate,
                            gst:distance.gst,
                            additional_gst:distance.additional_gst,
                            additional_rate:distance.additional_rate,
                            above:distance.above,
                            city_id:distance.city_id});

                            setTotal(distance.total);
                            setAdditionTotal(distance.additional_total);
   ...
   function edit(cell, row){
    ...
    const handleSubmit = e => {
        ...
        editDistance(chapter,token).then(data => {
            if (data.error) {
                setValues({ ...values, error: data.error, loading: false });
            } else {
                Router.push(`/slabs/distanceSlab`);
            }
        });
    ...
    return (
        ...
        <meta name="title" content='Edit Distance' />
    ...
    <div className="row">
        <div className="col-12">
            <div className="page-title-box">
                <h4 className="page-title float-left">Edit Distance</h4>

                <ol className="breadcrumb float-right">
                    <li className="breadcrumb-item"> <Link href='/dashboard'><a>Dashboard</a></Link></li>
                    <li className="breadcrumb-item active">Edit Distance</li>
                </ol>
...
<form role="form" onSubmit={handleSubmit}  >
    <div className="form-group">
        <label>From</label>
            <input type="text" className="form-control" value={from} placeholder="From in Km" id="name" name="name" onChange={handleChange('from')} required />
        ...
        <select id='single' className="form-control" value={city_id} name="city_id" required id="sel1" onChange={handleCityChange('city_id')} >
                    ...
                    <div className="form-group">
                    <label>Additional Rate / Km</label>
                   <input type="text" className="form-control" value={additional_rate} placeholder="Additional Rate / Km" id="name" name="name" onChange={handleAboveChange('additional_rate')}  />
                    </div>

                    <div className="form-group">
                    <label>GST / km</label>
                    <input type="text" className="form-control" value={additional_gst} placeholder="GST / km" id="name" name="name" onChange={handleAboveChange('additional_gst')}  />
                    </div>
                   ...
                   <div className="form-group">
                   <label>To</label>
                   <input type="text" className="form-control" value={to} placeholder="To in Km" id="name" name="name" onChange={handleChange('to')} required />
                    </div>
